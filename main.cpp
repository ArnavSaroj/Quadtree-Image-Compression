...
int main()
{
    ...
    for (int i = 0; i < numFrames; i++)
    {
        ...
        string outPath = "assets/processed/img21frame_" + to_string(i) + ".jpg";
        ...
        if (ok)
        {
            ...
            uintmax_t afterBytes = std::filesystem::file_size(outPath, sizeEc);
            if (!sizeEc && beforeBytes > 0)
            {
                double before = static_cast<double>(beforeBytes);
                double after = static_cast<double>(afterBytes);
                double savedPct = ((before - after) / before) * 100.0; // can be negative
